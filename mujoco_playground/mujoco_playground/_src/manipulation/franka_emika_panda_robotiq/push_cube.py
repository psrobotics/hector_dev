...
class PandaRobotiqPushCube(panda_robotiq.PandaRobotiqBase):
    ...
    def _get_rand_target_pos(self, rng: jax.Array, offset: jax.Array) -> jax.Array:
    ...
    def _get_rand_target_quat(self, rng: jax.Array, max_angle: jax.Array) -> jax.Array:
    ...
    def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
        ...
        state = mjx_env.State(state.data, obs, reward, done, state.metrics, state.info)
        return state
    ...
    def _get_success(
        ...
        state.info["prev_step_success"] = (success_cond_1 & success_cond_2).astype(int)
