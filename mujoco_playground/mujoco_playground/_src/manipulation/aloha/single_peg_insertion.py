...
class SinglePegInsertion(aloha_base.AlohaEnv):
    ...
    def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
        ...
        reward = sum(rewards.values()) / sum(self._config.reward_config.scales.values())
    ...
    def _get_reward(
        ...
        peg_dist = jp.linalg.norm(self._peg_end2_goal_pos - data.xpos[self._peg_body])
        ...
        peg_insertion_dist = jp.linalg.norm(
            data.site_xpos[self._peg_end2_site] - data.site_xpos[self._socket_rear_site]
        )
