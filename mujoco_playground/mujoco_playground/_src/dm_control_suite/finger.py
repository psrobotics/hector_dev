...
def _make_spin_model(xml_path: epath.Path, assets: Dict[str, Any]) -> mujoco.MjModel:
...
class Spin(mjx_env.MjxEnv):
    ...
    def _tip_position(self, data: mjx.Data) -> jax.Array:
        """Returns the (x,z) position of the tip relative to the hinge."""
        tip_xz = mjx_env.get_sensor_data(self.mj_model, data, "tip")[jp.array([0, 2])]
...
class Turn(mjx_env.MjxEnv):
    ...
    def __init__(
        ...
        self._xml_path = _XML_PATH.as_posix()
        self._model_assets = common.get_assets()
        self._mj_model = _make_turn_model(_XML_PATH, target_radius, self._model_assets)
        self._mj_model.opt.timestep = self.sim_dt
        self._mjx_model = mjx.put_model(self._mj_model, impl=self._config.impl)
        self._post_init()
