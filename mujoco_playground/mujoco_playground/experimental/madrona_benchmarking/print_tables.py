...
from mujoco_playground.experimental.madrona_benchmarking.benchmark import (
    MeasurementMode,
)
...
for env_name in ["CartpoleBalance", "PandaPickCubeCartesian"]:
    ...
    tm_winf = (
        1
        / df_env[df_env["measurement_mode"] == MeasurementMode.STATE_VISION_INF.value][
            "fps"
        ].mean()
    )
    tm_wvis = (
        1
        / df_env[df_env["measurement_mode"] == MeasurementMode.STATE_VISION.value][
            "fps"
        ].mean()
    )
    tm_state = (
    ...
    df_modes["Time per step"] = df_modes["Time per step"].apply(lambda x: f"{x:.6e}")
