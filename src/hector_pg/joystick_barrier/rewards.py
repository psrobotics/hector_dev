...
def _cost_joint_deviation_hip(self, context: Dict[str, Any]) -> jax.Array:
    qpos = context["q"]
    cost = jp.sum(
        jp.abs(qpos[self._hip_indices] - context["default_pose"][self._hip_indices])
    )
...
def _cost_contact_vel(self, context: Dict[str, Any]) -> jax.Array:
    ...
    mask = (
        jp.squeeze(first_contact, axis=-1) if first_contact.ndim == 2 else first_contact
    )
...
def _cost_stand_still(self, context: Dict[str, Any]) -> jax.Array:
    ...
    cmd_norm_track = jp.linalg.norm(
        commands[3:7] - jp.array([context["tar_body_height"], 0.0, 0.0, 0.0])
    )
